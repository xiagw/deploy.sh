...
### 3.1 编排模型：RUN 单数组（位置即依赖顺序）

> 本小节是主脚本流程调度的依赖注释。`execution-plan.md` 已并入本节，外部不再单独维护。
...
| v2 | `RUN_REPO`/`RUN_TASKS`/`RUN_STAGES`/`RUN_DEPLOY` 四数组 + main 三个循环点 | 可选函数按数组整体放置，独立功能被单循环点连坐，无法按真实依赖提前 |
| v3（当前） | `RUN` 单数组，**位置即依赖顺序**；`parse_args` 解析后直接组装；main 退化为一个 for 循环 | — |

v3 要点：

- 数组位置 = 执行顺序 = 依赖顺序，由 `parse_args` 一次组装完成（无独立 `_run_build_plan` 函数）。
- 必备步骤（config_deploy_init 等）也作为数组条目，保证「顺序」是唯一事实来源。
- main 正文退化为 `for fn in "${RUN[@]}"; do "$fn"; done`（RUN 最后一项恒为 `handle_notify`）。
- `RUN_DEPLOY` 保留为 `stage_deploy` 的选型查询表（非循环数组）。

组装位置在 `parse_args`（解析 + 组装一体）：解析只设 `arg_*` 标量与 `RUN_DEPLOY`，循环后按依赖顺序追加函数名。组装结束若未请求任何功能（`auto_mode=true`），追加全部阶段；`find_project_config` 的进入条件也依赖它。Gitea Actions 的 `setup_git_repo` 属环境驱动，不计入用户请求，不抑制自动模式；独立功能（`-r`/`-K`/`--clean-tags` 等）单独执行不触发自动模式。

### 3.2 启动与初始化（main 完整流程树）
...
├─ parse_args "$@"    # 解析参数 → arg_flags / deploy_method / arg_*，并组装 RUN
...
以下步骤无条件进入 RUN，相对顺序不可打乱（find_project_config 除外，见下）：

```
config_deploy_init  →  G_DATA/G_ENV、source deploy.env 加载 ENV_*、扩展 PATH
system_check        →  安装 git/curl/rsync 等基础工具、检测发行版
config_repo_vars  →  G_REPO_* / G_NAMESPACE / G_IMAGE_TAG / G_IMAGE_NAME（读 G_REPO_DIR 的 git 分支与 commit）
find_project_config →  G_CONF + PROJECT_BUILD_METHOD / PROJECT_DEPLOY_METHOD（读 G_REPO_GROUP_PATH）
...
- `config_repo_vars` 必须在仓库准备（setup_git_branch）之后：`get_git_branch` 优先读 CI 变量，本地场景回退 `git rev-parse`（repo.sh:447），分支切换必须先于该读取。
- `find_project_config`（若进入）必须在 `config_repo_vars` 之后：读 `G_REPO_GROUP_PATH`。仅在计划含 stage_build（读 PROJECT_BUILD_METHOD，build.sh:382）或 stage_deploy（读 G_CONF hosts / PROJECT_DEPLOY_METHOD，deployment.sh:702/791）时进入；独立功能与测试跳过，避免无关配置阻断或模板残留告警。
- `kube_config_init` 必须在 `config_repo_vars` 之后：读 `G_NAMESPACE`。
- `config_build_env` 计算 `IS_CHINA`，此前必须保持 unset：`_install_packages → _set_mirror`（common.sh:962）读 `IS_CHINA` 决定是否改写 apt 源。
...
config_deploy_init ─────────────────────────────────────────────┐
   ...
   │     └── setup_git_repo/svn/branch 【必须早于 config_repo_vars】│
   │                                                           ▼
