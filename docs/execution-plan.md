...
config_repo_vars  →  G_REPO_* / G_NAMESPACE / G_IMAGE_TAG / G_IMAGE_NAME
...
- `config_repo_vars` 必须在仓库准备（setup_git_branch）之后：`get_git_branch` 优先读 CI 变量，
  本地场景回退到 `git rev-parse`（repo.sh:447），分支切换必须先于该读取。
- `find_project_config`（若进入）必须在 `config_repo_vars` 之后：读 `G_REPO_GROUP_PATH`。
...
`parse_args` 负责「解析 + 组装」两件事，无独立 `_run_build_plan` 函数：

```
parse_args "$@"     # while 解析参数 → 设 arg_* 布尔/参数 + RUN_DEPLOY
                            # 循环后直接组装 RUN（位置即依赖顺序）
```
...
1. **仓库准备必须早于 config_repo_vars**：`get_git_branch`/`get_git_commit_sha` 在本地
