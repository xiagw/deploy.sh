...
Function Clear-UserCacheFiles
{
    ...
    ForEach ($localUser in (Get-ChildItem "$userRoot").Name)
    {
...
Function Clear-WindowsUserCacheFiles
{
    ...
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Internet Explorer\Cache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Internet Explorer\Recovery"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Internet Explorer\Tiles"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Terminal Server Client\Cache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\Caches"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\History\low"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\IECompatCache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\IECompatUaCache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\IEDownloadHistory"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\INetCache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\Temporary Internet Files"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\WebCache"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Windows\WER"
    Remove-Dir "$userRoot\$user\AppData\Local\Temp"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\Google\Chrome\User Data\Default" "Browser Google Chrome"
    Remove-Dir "$userRoot\$user\AppData\Local\Google\Chrome\User Data\SwReporter\"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\Microsoft\Edge\User Data\Default" "Browser Microsoft Edge"
    Remove-Dir "$userRoot\$user\AppData\Local\Microsoft\Edge\User Data\Default\CacheStorage"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-MozillaTemplate "$userRoot\$user\AppData\Local\Mozilla\Firefox\Profiles" "Browser Mozilla Firefox"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-MozillaTemplate "$userRoot\$user\AppData\Local\Waterfox\Profiles" "Browser Waterfox"
}
...
{
    param([string]$user = $env:USERNAME)
    if ((Test-Path "$userRoot\$user\AppData\Roaming\Microsoft\Teams"))
    {
        ...
        $teamsAppDataPath = (Get-ChildItem "$userRoot\$user\AppData\Roaming\Microsoft\Teams" | Where-Object { $_.Name -match "Default" }[0]).FullName
        ForEach ($cachePath in $possibleCachePaths)
        {
...
{
    param([string]$user = $env:USERNAME)
    Clear-MozillaTemplate "$userRoot\$user\AppData\Local\Thunderbird\Profiles" "Mozilla Thunderbird"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\EpicGamesLauncher\Saved\webcache" "Epic Games Launcher"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\Battle.net\BrowserCache" "BattleNet"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\Steam\htmlcache" "Steam"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\LolScreenSaver\cefCache" "Lol screen saver"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\Discord" "Discord"
}
...
{
    param([string]$user = $env:USERNAME)
    Clear-ChromeTemplate "$userRoot\$user\AppData\Local\AVG\User Data\Default" "Antivirus AVG"
}
...
{
    param([string]$user = $env:USERNAME)
    if (Test-Path $userRoot\$user\AppData\LocalLow\Google\GoogleEarth)
    {
        Get-ChildItem "$userRoot\$user\AppData\LocalLow\Google\GoogleEarth\unified_cache_leveldb_leveldb2\" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
        Get-ChildItem "$userRoot\$user\AppData\LocalLow\Google\GoogleEarth\webdata\" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
    }
}
...
{
    param([string]$user = $env:USERNAME)
    if ((Test-Path "$userRoot\$user\AppData\Local\Apple Computer\iTunes"))
    {
        $iTunesAppDataPath = "$userRoot\$user\AppData\Local\Apple Computer\iTunes"
...
{
    param([string]$user = $env:USERNAME)
    $DirName = "$userRoot\$user\AppData\LocalLow\Adobe\Acrobat"
...
{
    param([string]$user = $env:USERNAME)
    if ((Test-Path "$userRoot\$user\AppData\Local\Microsoft\Outlook"))
    {
        Get-ChildItem "$userRoot\$user\AppData\Local\Microsoft\Outlook\*.pst" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
        Get-ChildItem "$userRoot\$user\AppData\Local\Microsoft\Outlook\*.ost" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
        Get-ChildItem "$userRoot\$user\AppData\Local\Microsoft\Windows\Temporary Internet Files\Content.Outlook\*" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
        Get-ChildItem "$userRoot\$user\AppData\Local\Microsoft\Windows\Temporary Internet Files\Content.MSO\*" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
        Get-ChildItem "$userRoot\$user\AppData\Local\Microsoft\Windows\Temporary Internet Files\Content.Word\*" -Recurse -Force -ErrorAction SilentlyContinue |
                remove-item -force -recurse -ErrorAction SilentlyContinue -Verbose
    }
}
...
{
    param([string]$user = $env:USERNAME)
    $DirName = "$userRoot\$user\AppData\Roaming\LibreOffice"
    if ((Test-Path "$DirName"))
    {
