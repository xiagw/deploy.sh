...
main() {
    ...
    # shellcheck source=/dev/null
    [ -f "${SCRIPT_DATA}/aws.sh.env" ] && source "${SCRIPT_DATA}/aws.sh.env"
    ...
    # 如果没有指定 region，则从配置文件中读取，如果配置文件中也没有则使用默认值 "us-east-1"
    ...
    ec2) handle_ec2_commands "${args[@]}" ;;
    s3) handle_s3_commands "${args[@]}" 2>/dev/null || echo "错误：S3 服务模块尚未实现" >&2 ;;
    rds) handle_rds_commands "${args[@]}" 2>/dev/null || echo "错误：RDS 服务模块尚未实现" >&2 ;;
    vpc) handle_vpc_commands "${args[@]}" 2>/dev/null || echo "错误：VPC 服务模块尚未实现" >&2 ;;
    elb) handle_elb_commands "${args[@]}" 2>/dev/null || echo "错误：ELB 服务模块尚未实现" >&2 ;;
    iam) handle_iam_commands "${args[@]}" 2>/dev/null || echo "错误：IAM 服务模块尚未实现" >&2 ;;
    route53) handle_route53_commands "${args[@]}" 2>/dev/null || echo "错误：Route53 服务模块尚未实现" >&2 ;;
