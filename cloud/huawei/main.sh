...
main() {
    ...
    # shellcheck source=/dev/null
    [ -f "${SCRIPT_DATA}/huawei.sh.env" ] && source "${SCRIPT_DATA}/huawei.sh.env"
    ...
    # 如果没有指定 region，则从配置文件中读取，如果配置文件中也没有则使用默认值 "cn-north-1"
    ...
    obs) handle_obs_commands "${args[@]}" 2>/dev/null || echo "错误：OBS 服务模块尚未实现" >&2 ;;
