...
# 调用腾讯云 API
# 用法: call_tencent_api <service> <action> [参数...]
# 示例: call_tencent_api cvm DescribeInstances --region ap-guangzhou
call_tencent_api() {
    local service=$1
    shift
    local action=$1
    ...
    # 构建腾讯云 CLI 命令
    local tencent_cmd=("tccli" "$service" "$action")
    
    # 添加 profile 参数（如果指定）
    if [ -n "${profile:-}" ] && [ "$profile" != "default" ]; then
        tencent_cmd+=("--profile" "$profile")
    fi
    
    # 添加 region 参数（如果指定）
    if [ -n "${region:-}" ]; then
        tencent_cmd+=("--region" "$region")
    fi
    ...
    # 执行命令并输出 JSON
    "${tencent_cmd[@]}" --output json 2>/dev/null || "${tencent_cmd[@]}" --output json
}
...
generic_list() {
    ...
    if ! result=$(call_tencent_api "$service" "$api_action" "${api_args[@]}"); then
...
generic_create() {
    ...
    result=$(call_tencent_api "$service" "$api_action" "${api_args[@]}")
    local ret=$?
