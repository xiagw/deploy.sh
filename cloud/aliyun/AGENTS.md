...
- Central services ignore the user region: `cas`/`cdn`/`ram`/`bssopenapi`/`resourcemanager` calls must use `--region cn-hangzhou`; `alidns` calls must use `--region public` (cn-hangzhou also fails). `sts` has no region restriction. Because `call_aliyun_api` auto-appends `--region ${region}` when the caller omits it, central-service calls that omit `--region` break under `-r` of another region — always pass the fixed region explicitly.
