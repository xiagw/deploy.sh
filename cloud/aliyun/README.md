...
✅ **ECI** - 新增 fzf 支持（镜像、CPU、内存选择）
✅ **Ticket** - 新增 fzf 支持（产品、分类级联选择）
...
14. **MongoDB（云数据库 MongoDB）** - MongoDB 实例管理

### 其他服务
15. **CDN（内容分发网络）** - 域名管理、刷新、预热
16. **RAM（访问控制）** - 用户、权限管理
17. **CAS（证书服务）** - SSL 证书管理
18. **SMS（短信服务）** - 短信发送、状态查询
19. **ACR（容器镜像服务 个人版）** - 命名空间、仓库、镜像标签管理
20. **ECI（弹性容器实例）** - 容器实例管理
21. **Ticket（工单服务）** - 工单创建、回复、关闭
