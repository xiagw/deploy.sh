...
- 单元测试: 运行仓库中的 `tests/unit_test.sh`，并按语言自动调用测试框架（phpunit / mvn test / gradle test / npm test / pytest / go test 等）
...
- 功能测试: 运行仓库中的 `tests/func_test.sh`
- 性能测试: 使用 JMeter 执行仓库中的 `*.jmx` 测试计划
- 发布结果提醒到聊天软件: 企业微信, Telegram, Element(Matrix), Email, Zoom, 飞书...
